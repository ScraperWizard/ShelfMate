import { Command } from "../../Applications/Commands/Context.js";
...
const CommandRouter = async (
  ...
  console.log(
    `Received command ${Command.getName()} from client ${
      Client.getName() || Client.getSocketId()
    }`
  );
  ...
  if (
    Command.getUserAccessLevel() > Client.getAccessLevel()
  ) {
  ...
  if (data?.notification) {
    Socket.emit("show-notification", data.notification as Notification);
